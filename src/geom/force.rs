...
///
/// let origin = Vec3D::from(0.0, 0.0, 0.0);
/// let point = Vec3D::from(2.0, 1.0, 0.0);
///
...
/// );
/// ```
impl Mul<V> for Vec3D {
    type Output = Vec3D;

    fn mul(self, other:V) -> Vec3D {
        Vec3D {
            x: self.x * other,
            y: self.y * other,
            z: self.z * other,
